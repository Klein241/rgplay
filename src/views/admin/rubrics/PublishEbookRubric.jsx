...
import {
  ...
  Image as ImageIcon, DollarSign, Tag, X
} from 'lucide-react';
...
export const PublishEbookRubric = ({
  ...
  // ── 1. MODIFICATION PRIX & POINTS EN MASSE (E-BOOKS) ──
  const handleApplyBulkPriceAndPoints = async () => {
    ...
    try {
      ...
      const res = await apiClient?.bulkUpdateAudiobooks(selectedEbookIds, { price: p, unlock_points: pts });
      if (res?.success) {
        setBooks(prev => prev.map(b => selectedEbookIds.includes(b.id) ? { ...b, price: p, unlock_points: pts } : b));
        window.dispatchEvent(new CustomEvent('rg:library-updated'));
        setBulkSuccessMsg(`✅ ${selectedEbookIds.length} livre(s) mis à jour : ${p} FCFA • ${pts} Points !`);
        setTimeout(() => {
          setBulkSuccessMsg('');
  ...
  // ── 2. MODIFICATION TYPE EN MASSE (E-BOOKS) ──
  const handleApplyBulkType = async () => {
    ...
    try {
      ...
      const res = await apiClient?.bulkUpdateAudiobooks(selectedEbookIds, updates);
      if (res?.success) {
        setBooks(prev => prev.map(b => selectedEbookIds.includes(b.id) ? { ...b, ...updates } : b));
        window.dispatchEvent(new CustomEvent('rg:library-updated'));
        setBulkSuccessMsg(`✅ ${selectedEbookIds.length} contenu(s) converti(s) en "${bulkType}" !`);
        setTimeout(() => {
          setBulkSuccessMsg('');
          setBulkActionModal(null);
          setSelectedEbookIds([]);
          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            {ebookSubTab === 'list' && (
              <div className="space-y-6 animate-fadeIn">
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                      <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        <div>
                          <p className="text-sm font-extrabold text-white flex items-center gap-2">
                            <span>{selectedEbookIds.length} livre{selectedEbookIds.length > 1 ? 's' : ''} sélectionné{selectedEbookIds.length > 1 ? 's' : ''}</span>
                            <span className="text-[10px] px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300 border border-purple-400/30">
                              Actions groupées
                            </span>
                          </p>
                          <p className="text-xs text-slate-400">
                            Modifiez le prix, les Sky Points ou le type pour tous les livres sélectionnés
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto justify-end">
                        ...
                        <button
                          ...
                          title="Changer le format (Audiobook / E-Book / Podcast)"
                        >
                ...
                {/* ── MODAL 1 : MODIFICATION PRIX & POINTS EN MASSE (E-BOOKS) ── */}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400">Appliquer à {selectedEbookIds.length} livre(s) sélectionné(s)</p>
                          </div>
                        </div>
                        <button
                      ...
                      <div className="flex items-center justify-end gap-3 pt-3 border-t border-white/10">
                        ...
                        >
                          {isBulkUpdating ? (
                            ...
                            <>
                              <Check className="w-4 h-4" />
                              <span>Appliquer à {selectedEbookIds.length} livre(s)</span>
                            </>
                          )}
                        </button>
                      </div>
                ...
                {/* ── MODAL 2 : CHANGER LE TYPE / FORMAT EN MASSE (E-BOOKS) ── */}
